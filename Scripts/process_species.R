...
sp <- read.csv("/Users/morganehenry/Documents/GitHub/EU_extrapolation/data/species.csv", header=T)

##Fill blank with NA
sp[sp == ""] <- NA 

##Separate column "taxon" on genus, species, subspecies
df.sp <- separate(sp, col= "Taxon", c("genus", "Species", "other","other1","other2","other3","other4","other5","other6","other7"), sep=" ")
sp$genus <- df.sp$genus
sp$Species <- paste(df.sp$genus, df.sp$Species)
...
##Create new column names Official_country to match InvaCost column
sp$Official_country <- sp$Location

##Change Czech Republic to Czechia in sp
sp$Official_country[sp$Location == "Czech Republic"] <- "Czechia"

territories <- c("Azores", "Balearic Islands", "Canary Islands", "Faroe Islands", "Gibraltar", "Guernsey", "Isle of Man", "Jersey", "Madeira", "Svalbard and Jan Mayen")
terr1 <- which(sp$Location %in% territories)
sp <- sp[-terr1,]
